...
(function () {
    ...
    class AdminGrid {
        constructor(gridEl) {
            ...
            this.isDirty = false;
            this.toolbar = null;
            this.editableEligible = new Set();
            // Pending editable-column selection — batched until profile Save,
            // exactly like visible/order/width. Seeded from the server on load.
            this.serverEditable = [];
            this.editableSelected = new Set();
            this.editableDirty = false;
        ...
        async init() {
            ...
            // Ask the backend which columns are editable-eligible (native + custom)
            await this.fetchEditableEligible();

            // Seed the pending editable selection from the persisted server state.
            this.editableSelected = new Set(this.serverEditable);

            // Build toolbar UI
            this.buildToolbar();

            // Inject inline-edit pencils onto editable cells
            this.injectPencils();
        ...
        async loadFromServer() {
            ...
            try {
                ...
                this.serverEditable = data.editableColumns || [];
        ...
        _buildGridColumnRows(container) {
            ...
            ordered.forEach(col => {
                ...
                // Editable toggle — shown for any column the backend deems
                // editable-eligible (native product attributes AND custom EAV
                // columns). The server, not a name prefix, decides. Batched:
                // toggling only updates the pending set until profile Save.
                if (this.editableEligible.has(col.code)) {
                    const editWrap = document.createElement('label');
                    editWrap.className = 'admingrid-editable-toggle';
                    editWrap.title = 'Make cells inline-editable';
                    const editCb = document.createElement('input');
                    editCb.type = 'checkbox';
                    editCb.checked = this.editableSelected.has(col.code);
                    editCb.addEventListener('mousedown', e => e.stopPropagation());
                    editCb.addEventListener('change', () => {
                        this.toggleEditable(col.code, editCb.checked);
                    });
                    editWrap.appendChild(editCb);
                    const pencilIcon = document.createElement('span');
                    pencilIcon.textContent = '✎';
                    pencilIcon.className = 'admingrid-editable-icon';
                    editWrap.appendChild(pencilIcon);
        ...
        async saveCurrentProfile() {
            ...
            try {
                const data = await this.postAction('saveProfile', {
                    ...
                    is_default: '1',
                    editable_columns: JSON.stringify([...this.editableSelected]),
                ...
                if (data.success) {
                    ...
                    this.saveToCache();
                    this.flash('Saved');
                    this.serverEditable = [...this.editableSelected];
                    // Reload only if editability changed, so the grid re-renders
                    // its cells with the updated pencils.
                    if (this.editableDirty) {
                        this.editableDirty = false;
                        this.reloadGrid();
                    }
                }
        ...
        /**
         * Ask the backend which of the current grid columns are editable-eligible.
         * Populates this.editableEligible (Set of column codes). The backend is the
         * authority — the JS never infers eligibility from a name prefix.
         */
        async fetchEditableEligible() {
            const codes = this.columns.map(c => c.code).filter(c => c && c !== 'massaction');
            if (codes.length === 0) { this.editableEligible = new Set(); return; }
            try {
                const url = this.getConfigUrl('editableEligible')
                    + `?grid_block_id=${encodeURIComponent(this.gridBlockId)}`
                    + `&codes=${encodeURIComponent(codes.join(','))}`;
                const data = await mahoFetch(url, { loaderArea: false });
                this.editableEligible = new Set(data.codes || []);
            } catch (e) {
                console.warn('AdminGrid: eligibility load failed', e);
            }
        }

        /**
         * Add a hover "edit" pencil to every editable cell (td.admingrid-editable).
         * Server marks those cells; here we only attach the affordance + handler.
         */
        injectPencils() {
            if (!this.dataTable) return;
            this.dataTable.querySelectorAll('tbody td.admingrid-editable').forEach(td => {
                if (td.querySelector('.admingrid-pencil')) return;
                if (td.classList.contains('admingrid-editing')) return;

                const pencil = document.createElement('span');
                pencil.className = 'admingrid-pencil';
                pencil.title = 'Edit';
                pencil.textContent = '✎';
        ...
        /** Resolve a cell's column code from its position and the header row. */
        getColumnCodeForCell(td) {
            const row = td.parentElement;
            if (!row) return null;
            const idx = Array.prototype.indexOf.call(row.children, td);
            const headerRow = this.dataTable?.querySelector('thead tr.headings');
            const th = headerRow ? headerRow.children[idx] : null;
            return th ? th.getAttribute('data-column-id') : null;
        }

        /** Resolve the entity (row) id for a cell: massaction checkbox, then row url. */
        getRowEntityId(td) {
            const row = td.closest('tr');
            if (!row) return null;
            const cb = row.querySelector('input.massaction-checkbox, input[type=checkbox][value]');
            if (cb && /^\d+$/.test(cb.value)) return cb.value;
            const m = (row.getAttribute('title') || '').match(/\/id\/(\d+)\//);
            return m ? m[1] : null;
        }

        /** Current store scope from the admin store switcher (0 = default). */
        getStoreId() {
            const sw = document.getElementById('store_switcher');
            if (sw && /^\d+$/.test(sw.value)) return sw.value;
            return '0';
        }

        /** Fetch (and cache) editor metadata for a column at the current store. */
        async fetchEditorMeta(code, storeId) {
            this._metaCache = this._metaCache || {};
            const key = `${code}@${storeId}`;
            if (this._metaCache[key]) return this._metaCache[key];

            const url = this.getConfigUrl('editorMeta')
                + `?grid_block_id=${encodeURIComponent(this.gridBlockId)}`
                + `&column_code=${encodeURIComponent(code)}`
                + `&store_id=${encodeURIComponent(storeId)}`;
            const data = await mahoFetch(url, { loaderArea: false });
            this._metaCache[key] = data;
            return data;
        }

        /** Swap a cell into inline-edit mode. */
        async startEdit(td) {
            if (td.classList.contains('admingrid-editing')) return;

            const code = this.getColumnCodeForCell(td);
            const entityId = this.getRowEntityId(td);
            if (!code || !entityId) return;
            const storeId = this.getStoreId();

            let meta;
            try {
                meta = await this.fetchEditorMeta(code, storeId);
            } catch (e) {
                console.error('AdminGrid: editor meta failed', e);
                return;
            }
            if (!meta || !meta.editable) return;

            // Preserve the current display so Cancel / errors can restore it.
            const originalHtml = td.innerHTML;
            const currentText = (td.querySelector('.admingrid-pencil')
                ? td.textContent.replace('✎', '')
                : td.textContent).trim();

            td.classList.add('admingrid-editing');

            const editor = document.createElement('div');
            editor.className = 'admingrid-editor';
            ...
            if (meta.input === 'select') {
                ...
                (meta.options || []).forEach(o => {
                    ...
                    if (o.label.trim() === currentText) opt.selected = true;
            ...
            field.className = 'admingrid-editor-field';
            editor.appendChild(field);

            if (meta.scopeLabel) {
                const scope = document.createElement('span');
                scope.className = 'admingrid-editor-scope';
                scope.textContent = meta.scopeLabel;
                editor.appendChild(scope);
            }

            const save = document.createElement('button');
            save.type = 'button';
            save.className = 'admingrid-editor-save';
            save.title = 'Save';
            save.textContent = '✓';

            const cancel = document.createElement('button');
            cancel.type = 'button';
            cancel.className = 'admingrid-editor-cancel';
            cancel.title = 'Cancel';
            cancel.textContent = '✗';

            const msg = document.createElement('span');
            msg.className = 'admingrid-editor-msg';

            const restore = () => {
                td.classList.remove('admingrid-editing');
                td.innerHTML = originalHtml;
                if (!td.querySelector('.admingrid-pencil')) this.injectPencils();
            };
            ...
            const commit = async () => {
                ...
                msg.textContent = '';
                try {
                    const data = await this.postAction('inlineEdit', {
                        grid_block_id: this.gridBlockId,
                        column_code: code,
                        entity_id: entityId,
                        value: field.value,
                    ...
                    if (data.success) {
                        td.classList.remove('admingrid-editing');
                        td.textContent = data.value != null ? data.value : field.value;
                        this.injectPencils();
                    } else {
        ...
        /**
         * Toggle a column's pending editable state. Batched like the Visible
         * checkbox — nothing is persisted here; the change is written on the next
         * profile Save (grid pencils update after that Save-triggered reload).
         */
        toggleEditable(code, editable) {
            if (editable) {
                this.editableSelected.add(code);
            } else {
                this.editableSelected.delete(code);
            }
            this.isDirty = true;
            this.editableDirty = true;
        }
        ...
        onGridReloaded() {
            ...
            // Re-inject inline-edit pencils (grid HTML was replaced)
            this.injectPencils();

            // Refresh eligibility (columns may have changed) and, if the Columns
            // dropdown is open/built, rebuild its rows so toggles stay accurate.
            this.fetchEditableEligible().then(() => {
                const gridCols = this._columnsDropdown?.querySelector('.admingrid-grid-cols');
                if (gridCols) this._buildGridColumnRows(gridCols);
            });
        }
    }
